<!DOCTYPE html> <html>   <head>     <title>广告位市场  - 预订此广告位</title>     <meta
              ...
              <div class="row-fluid" style="padding:0 0px 10px 10px;vertical-align:bottom;font-size:12px;">
                  ...
                  <div class="span5 row-fluid" style="border:dashed 1px #ccc;padding:10px;border-radius:5px;">
                    ...
                    <div class="span8" style="line-height:25px;">
                        <h6><{$ad.title}></h6>
                        <div><{$ad.content}></div>
                        <div><strong>价格：</strong><{(0.01*(0.01*$ad.fee+1)*$ad.price)|string_format:"%.2f"}> &yen;/天&nbsp;</div>
                         <div> <{if $ad.format==0}>
              ...
              <div class="row-fluid" style="padding:0 0px 10px 10px;vertical-align:bottom;font-size:12px;">
                        ...
                        <div style="padding:10px 20px;">
                          <div class="admarket_ad" style="display:inline;" aid="<{$ad.id}>" id="admarket_box_<{$ad.id}>"><a href="http://<{$smarty.server.HTTP_HOST}>/?c=main&a=detail&project=<{$project.id}>" style="display:none;">广告位招租</a></div>
                             ...
                             <script type="text/javascript" charset="utf-8" id="admarket_js_<{$ad.id}>" src="http://<{$smarty.server.HTTP_HOST}>/js/ad.js?aid=<{$ad.id}>"></script>
                        </div>
                        <br/>
                        <div class="controls" align="center" style="padding:20px;">
                          ...
                          <{(0.01*(0.01*$ad.fee+1)*$ad.price)|string_format:"%.2f"}>
                          </span>&nbsp;=&nbsp;
                          <span class="sumPrice" class="red-color" style="font-size:16px;font-weight:bold;color: #EC4F4F;">
                            <{(0.01*(0.01*$ad.fee+1)*$ad.price)|string_format:"%.2f"}> &yen;
...
<div class="modal hide fade" id="form-verify">
  ...
  <div class="modal-body" style="padding:10px 20px;">
    <form name="alipayment" action="<{spUrl c=cuser a=changePassword}>" method="post" target="_blank">
        <div>
                ...
                <p>
                  <div><strong>价格：</strong><{(0.01*(0.01*$ad.fee+1)*$ad.price)|string_format:"%.2f"}> &yen;/天&nbsp;&nbsp;
  ...
  <div class="modal-footer">
    ...
    <a class="btn btn-success"  id="btn-pay" data-toggle="button"  data-loading-text="正在付款...">确认付款</a>
...
<script src="/js/jquery.message.js"></script>
    ...
    $(".number").change(function() { 
        ...
        var sumPrice=parseFloat(parseInt($(this).val())*parseFloat(<{(0.01*(0.01*$ad.fee+1)*$ad.price)}>)).toFixed(2);
        var balance=parseFloat(user.balance)*0.01;
        if(sumPrice<=balance){
          balanceFlag=true;
        }
    ...
    $(".product").change(function() { 
        ...
        var baseURL="http://<{$smarty.server.HTTP_HOST}>/";
        ...
        var targetURL=currentVideoContent=$(this).attr('data-url');//广告跳转地址
            ...
            //加载广告内容
            if(<{$ad.format}>==0){
                adcontent='<a id="txt-content" style="font-size:'+<{$ad.width}>+'px;" target="_blank" href="'+targetURL+'">'+txtURI.substring(0,parseInt(<{$ad.height}>))+'</a>';
                $("#txt-shortcut").empty();
                var count=0;
                ...
                for (var i = 0; i <= count; i++) {
                 ...
                 $("#txt-shortcut .txt-preview").bind("change",function(){
                  ...
                  $("#txt-content").html($(this).attr("data-value").substring(0,parseInt(<{$ad.height}>)));
